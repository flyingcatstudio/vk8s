...
노드를 클릭하면 **그 노드와 직접 연결된 것만** 선명하게 남고 나머지는 흐려진다(`FADE = 0.18`).
같은 노드를 다시 클릭하면 해제, 다른 노드를 클릭하면 포커스가 옮겨간다.

**직접 연결 = 1 hop.** `A → B → C`에서 A를 누르면 A와 B만 켜진다 — C도, B에 붙은 다른 노드도
켜지지 않는다. 전이 폐포(연결된 컴포넌트 전체)로 시작해 2-hop으로 줄였다가 여기까지 왔다.
"이 노드가 무엇과 붙어 있나"를 묻는 그림이므로 한 칸이 정답이다.

엣지는 양쪽 끝이 모두 켜진 것만 선명하게 유지한다.
...
| 키 | 동작 | 비고 |
|---|---|---|
| `Ctrl/Cmd + S` | 저장 (localStorage) | 자동 저장이 이미 돌지만 즉시 저장하고 싶을 때 |
| `Delete` | 선택한 엔티티 삭제 | 트리/인스펙터에서 선택한 것. **입력란에 커서가 있으면 동작하지 않음** |
| `Enter` / `Esc` | 체크리스트 항목 인라인 편집 커밋 / 취소 | 편집 중인 입력란 안에서만 |

> **문서 정정** — 이전 표에 있던 `[` · `]` · `Ctrl+B` · `Esc`(선택 해제)는 실제로 구현되어 있지 않다.
> [`docs/ui-spec.md` §10](./docs/ui-spec.md)의 표(`Ctrl+Z` undo/redo · `Ctrl+K` 트리 검색 · `1`–`6` 뷰 전환 등)는
> **설계 스펙**이며 현재 런타임에는 `keydown` 리스너가 위 세 가지뿐이다.

### 삭제 수단

| 위치 | 방법 |
|---|---|
| **좌측 트리** | 행에 마우스를 올리면 나타나는 🗑 (사이트 · 노드 · L4 · 스토리지 · 네트워크 장비 · 외부 서비스 · 사이트 연결 · 클러스터/VM 그룹 · 워크로드/서비스) |
| **인스펙터 헤더** | 우측 끝 🗑 |
| **키보드** | 선택 후 `Delete` |

삭제는 항상 확인을 받는다. 잠긴(🔒) 클러스터·워크로드는 문구가 다르고, 노드를 가진 사이트는
"노드 N개도 함께 삭제됩니다" 확인이 따로 뜬다. 클러스터를 지워도 **노드는 삭제되지 않고
unassigned로 돌아간다** (다른 클러스터가 여전히 참조하면 역할을 유지).
